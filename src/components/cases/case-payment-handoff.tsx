import { ArrowLeft, CheckCircle2, Coins, LoaderCircle, LockKeyhole, RotateCcw } from "lucide-react";
  ...
  return (
    <section className="overflow-hidden rounded-2xl border border-[#d9dfd3] bg-white shadow-[0_18px_55px_rgba(31,39,27,0.07)]">
      <div className="grid gap-8 p-7 sm:p-9 lg:grid-cols-[1fr_260px] lg:items-center">
        <div>
          ...
          <p className="mt-6 text-[11px] font-bold uppercase tracking-[0.16em] text-[#718218]">
            {analyzing ? "Building report" : status === "analysis_failed" ? "Analysis needs attention" : unlocked ? "Prospect workflow reserved" : "Prospect analysis"}
          </p>
          <h1 className="mt-2 max-w-xl text-3xl font-bold tracking-[-0.035em] text-[#172017]">
            {analyzing ? "Your evidence-backed report is being built." : status === "analysis_failed" ? "This attempt stopped safely." : unlocked ? "This Prospect Case is already covered." : "Start the complete Prospect workflow."}
          </h1>
        ...
        <div className="rounded-2xl border border-[#dfe5d8] bg-[#f7f9f3] p-5">
          <div className="flex items-center justify-between border-b border-[#dfe5d8] pb-4">
            <span className="text-xs font-bold text-[#657065]">Prospect workflow</span>
            <span className="text-xl font-bold text-[#1a231a]">1 credit</span>
          </div>
          <ul className="mt-4 space-y-3 text-xs leading-5 text-[#667266]">
            <li className="flex gap-2"><CheckCircle2 className="mt-0.5 shrink-0 text-[#76970d]" size={14} />Discovery and report included</li>
            <li className="flex gap-2"><RotateCcw className="mt-0.5 shrink-0 text-[#76970d]" size={14} />Refresh and resume without another charge</li>
            <li className="flex gap-2"><LockKeyhole className="mt-0.5 shrink-0 text-[#76970d]" size={14} />Technical failure returns the credit</li>
          </ul>
          {!unlocked && (
            <button
              type="button"
            ...
            >
              {status === "creating_checkout" ? <LoaderCircle className="animate-spin" size={17} /> : <Coins size={17} />}
              Start analysis · uses 1 credit
            </button>
          )}
          {unlocked && status !== "analysis_failed" && (
            <div className="mt-5 rounded-xl bg-[#eaf4cf] px-4 py-3 text-center text-xs font-bold text-[#56720c]">
              {analyzing ? "Collecting and validating evidence…" : "Already covered · no second charge"}
            </div>
          )}
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-t border-[#e8ece4] bg-[#fbfcfa] px-7 py-4 sm:px-9">
        ...
        <span className="text-[11px] font-semibold text-[#8a948a]">No subscription · credits never expire</span>
      </div>
    </section>
  );
}
