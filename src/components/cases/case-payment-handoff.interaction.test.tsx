...
describe("CasePaymentHandoff interactions", () => {
  it("starts analysis only from the ready action and keeps the Case scope visible", async () => {
    ...
    const start = screen.getByRole("button", { name: "Start analysis · uses 1 credit" });
    expect(start).toBeEnabled();
    await user.click(start);
    expect(onCheckout).toHaveBeenCalledOnce();
  });

  it("disables a busy start and exposes compensated analysis retry copy", async () => {
    const user = userEvent.setup();
    const retry = vi.fn();
