...
export function parseDraft(value: unknown, now = new Date()): NewCaseDraft | null {
  ...
  if (!(value.previous_analysis_job_id === undefined || value.previous_analysis_job_id === null || (typeof value.previous_analysis_job_id === "string" && uuidV4.test(value.previous_analysis_job_id)))) return null;
  ...
  return {
    ...
    previous_analysis_job_id: value.previous_analysis_job_id ?? null,
