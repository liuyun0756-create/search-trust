...
const SENSITIVE_KEYS = new Set([
  "authorization", "proxy_authorization", "cookie", "set_cookie", "signature",
  "svix_signature", "code", "state", "pkce_verifier", "code_verifier",
  "access_token", "refresh_token", "id_token", "google_token", "client_secret",
  "service_role_key", "api_key", "secret", "email", "account_email", "phone",
  "address", "site_url", "website_url", "search_query", "query_text", "payload",
...
function sanitize(value: unknown, seen: WeakSet<object>, depth: number): unknown {
  ...
  if (value instanceof Error) return { name: sanitizeString(value.name), message: sanitizeString(value.message) };
...
export function safeLogError(error: unknown): { name: string; message: string } {
  if (error instanceof Error) {
    return { name: sanitizeString(error.name), message: sanitizeString(error.message) };
  }
