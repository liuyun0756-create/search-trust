...
import { GbpSyncRequestError, type GbpSyncService } from "./gbp-service";

export function createGbpSyncHandlers(deps: { enabled(): boolean; user(): Promise<{ userId: string } | null>; service(): GbpSyncService }) {
  type Context = { params: Promise<{ id: string }> };
  const headers = { "cache-control": "no-store" };
  function uuid(value: unknown) {
    try { return parseUuid(value); } catch { throw new GbpSyncRequestError("SYNC_INVALID_REQUEST", 400); }
  }
  async function run(request: NextRequest, context: Context) {
    try {
      if (!deps.enabled()) throw new GbpSyncRequestError("SYNC_DISABLED", 503);
      const user = await deps.user();
      if (!user) throw new GbpSyncRequestError("SYNC_FORBIDDEN", 401);
      ...
      if (origin && origin !== request.nextUrl.origin) throw new GbpSyncRequestError("SYNC_FORBIDDEN", 403);
      let input;
      try {
      ...
      } catch { throw new GbpSyncRequestError("SYNC_INVALID_REQUEST", 400); }
      ...
      const safe = error instanceof GbpSyncRequestError ? error : new GbpSyncRequestError("SYNC_STORAGE_UNAVAILABLE", 503);
